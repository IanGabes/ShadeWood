...
Main.Game.prototype=
{
    ...
    preload: function () 
    {
        ...
        this.game.load.spritesheet('bear', 'img/bear_fullSpriteSheet.png',496,369);
        this.game.load.spritesheet('deer','img/deer_fullSpriteSheet.png',250,354);
        ...
        if(game.input.keyboard.isDown(Phaser.Keyboard.P) && ghost.key == 'ghost'){
            for(i = 0; i < 5;i++)
            {
               if(game.physics.arcade.intersects(ghost.body, enemies[i].body))
               {
                    ...
                    ghost = enemies[i];
                    ghost.loadTexture(enemies[i].animations.currentAnim.name);
    ...
    depossess: function(){
        ...
        ghost = game.add.sprite(x, y, 'ghost');
        ghost.enableBody = true;
        game.physics.enable(ghost,Phaser.Physics.ARCADE);
        ghost.collideWorldBounds = true;
        //ghost.fixedToCamera = true;
        ghost.animations.add('idle', [8,7,6,5,4], 7, true);
        ghost.animations.add('possess',[17,16,15,14,13],6,true);
        ghost.animations.add('neophyte',[26,25,24,23],7,true);
        ghost.animations.add('transform',[35,34,33,32,31,30,29,28,27],9,true);
        ghost.animations.add('depossess',[13,14,15,16,17],6, true);
        ghost.anchor.setTo(.5,1);
        ghost.body.gravity.y = 0;
        ghost.body.allowGravity = false;

        ghost.animations.play('depossess',6,false);
        ghost.animations.play('idle',6,true);
